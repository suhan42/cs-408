...
则称图 G 为简单图。上图中$G_1$和$G_2$均为简单图。【注意】数据结构中仅讨论简单图。
...
对于无向图， |E| 的取值范围是 0 到$\cfrac {n(n-1)} {2}$，有$\cfrac {n(n-1)} {2}$条边的无向图称为完全图。

**就是所有两个顶点都有边。**

对于有向图，|E|的取值范围是 0 到 n(n-1)，有 n(n-1) 条弧的有向图称为有向完全图，在**有向完全图**中任意两个顶点之间都存在方向相反的两条弧。

上图中$G_2$为无向完全图，而下面$G_3$为有向完全图。
...
==生成子图==：若有满足 V(G')= V(G) 的子图 G'，则称其为 G 的**生成子图**。上图中 G3 为 G1 的子图。
...
若图 G 中**任意两个顶点都是连通的**，则称图G为**连通图**，否则称为**非连通图**。

**无向图中的<u>极大连通子图</u>称为<u>连通分量</u>**。若一个图有n个顶点，并且边数小于n - 1，则此图必是非连通图。如下图(a)所示，图G4有3个连通分量，如图(b)所示。

![6连通图.png](imgs-DS/6连通图.png)

**极大连通子图**是无向图的连通分量，极大即要求该连通子图包含其所有的边。
...
在**有向图**中，若从顶点v到顶点w和从顶点w到项点v之间都有路径，则称这两个顶点是强连通的。
...
==入度==是以顶点**v为终点**的有向边的数目，记为 **ID(v)**。

==出度==是以顶点**v为起点**的有向边的数目，记为 **OD(v)**。
...
> 比如：
>
> A^2^\[1]\[4]：就是从A->B，然后B->D，这样两次（路径长度为2）的。
>
> A^2^\[1]\[4] = 1，意思就是满足长度为2的路径，只有一条。
...
|                    | 邻接矩阵                             | 邻接表                                              |
| ------------------ | ------------------------------------ | --------------------------------------------------- |
| 空间复杂度         | O( IVI^2^ )                          | 无向图O(\|V\|+2\|E\|)；<br />有向图O(\|V\|+\|E\|)。 |
| 适用于             | 稠密图                               | 稀疏图                                              |
| 表示方式           | 唯一                                 | 不唯一                                              |
...
2. 因为邻接矩阵的表示方式是唯一的，所以DFS算法得到的遍历序列是唯一的。但是因为单链表的表示方式不是唯一的，所以DFS算法得到的遍历序列不是唯一的。
...
一个图可以有多个生成树，我们定义无向连通图的 **最小生成树**（Minimum Spanning Tree，MST）为**边权和最小**的生成树。
...
![prim](https://img-blog.csdnimg.cn/20210302161257975.png)

- Prim算法只和顶点个数有关系，它的时间复杂度是**O(|V|^2^)**。
- 适合于边**稠密的图**。
...
![Kruskal](https://img-blog.csdnimg.cn/20210302165922363.png)

- Kruskal算法只关系边的个数，它的时间复杂度是**O(|E|log2|E|)**。
- 适合于边**稀疏的图**。
...
> ![image-20240605092759512](imgs-DS/6kruskal算法.png)
...
Dijkstra算法[1]用于**构建单源点**的最短路径—，即图中某个点到任何其他点的距离都是最短的。例如，构建地图应用时查找自己的坐标离某个地标的最短距离。可以用于有向图，但是不能存在负权值。

[6.4_3_最短路径问题_Dijkstra算法_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1b7411N798/?p=66&spm_id_from=pageDriver&vd_source=9d3c5a863c7c6dbd5152dd7c4e9dc492)

![Dijstra算法](imgs-DS/6Dijstra算法.png)

![在这里插入图片描述](https://img-blog.csdnimg.cn/20210302192004317.png)
...
![6fuloyd例子1](imgs-DS/6fuloyd例子1.png)

1. 初始化：方阵$A^{(-1)}[i][j]=arcs[i][j]$.

2. 第一轮：将$V_0$作为中间顶点，对于所有顶点$\{i,j\}$，如果有$A^{-1}[i][j]>A^{-1}[i][0]+A^{-1}[0][j]$，则将$A^{-1}[i][j]$更新为$A^{-1}[i][0]+A^{-1}[0][j]$.
...
![6fuloyd例子2](imgs-DS/6fuloyd例子2.png)

从这个表中，可以发下一些规律：

![6fuloyd例子3](imgs-DS/6fuloyd例子3.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20210712160902234.png)
...
从源点到汇点的有向路径可能有**多条**，所有路径中，具有**最大路径长度**的路径称为**关键路径**，而把关键路径上的活动称为**关键活动**。
...
![艾兹赫尔·戴克斯特拉](https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Edsger_Wybe_Dijkstra.jpg/800px-Edsger_Wybe_Dijkstra.jpg)
