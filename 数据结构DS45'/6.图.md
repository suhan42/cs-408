...
1. ==初始化：是把一个顶点放入并查集。==
2. ==根据并查集内的点，找到能走**一步**到达的路径，并把这些走一步的路径里面的路径**权值最小**的那个到达的点放入并查集，并锁定这条路径（就是**最短路径**）。==
3. ==结束：直到全部的点都在并查集结束。==
...
1. **一定是==无环图==才有拓扑排序，有环则不行。**如果输出顶点数少了，哪怕是少了一个，也说明这个网存在环(回路)，不是AOV网。
...
2. 当网中的**关键路径并不唯一**，对于有几条关键路径的网，只提高一条关键路径上的关键活动速度并不能缩短整个工程的工期，只有加快那些**包括在所有关键路径上的==共有的==关键活动（边权值）**才能达到缩短工期的目的。
...
| :----------------------------------------------------------- | ---- | ------ | ---- | ----- | ----- | ---- | ------ |
| 正向（拓扑排序）推算事件(结点)==最**早**==发生的时间$Ve$<br />（如果有两个前驱得到两个值，==**选大的**==，因为早完成的需要等晚完成的） | $Ve$ | 正向0  | 3    | 2     | 6     | 6    | 8      |
| 反向（逆拓扑排序）推算事件(结点)==最**晚**==发生的时间$Vl$<br />（如果有两个后继得到两个值，==**选小的**==，因为小的表示早完成的那个，需要早完成后继才能正常进行）<br />**【注意】源点、汇点是不变的**，就是上面求的最早发生时间的数字。 | $Vl$ | 反向0  | 4    | 2     | 6     | 7    | 8      |
| $Vd = Vl - Ve$<br />Vd为0，表示没有商量的余地，所以是**关键事件**。 | $Vd$ | **0**  | 1    | **0** | **0** | 1    | **0**  |

==【考点：求关键路径长度】==如果是求关键路径长度，那么只需要正向拓扑排序算出汇点的值即可，不需要再求逆向的，找关键事件。

把vd=0的关键事件结点都带上：所以==关键路径==是：V1->V3->V4->V6。
